...
async function waitForServer() {
  ...
  if (!res) throw new Error('Health check request failed (server not reachable)');
  const text = await res.text();
  logStep('GET /health', '200 with status json', `${res.status} ${text.slice(0, 200)}...`);
...
async function ensureTestCategory() {
  ...
  logStep('Create Category', 'Created with unique id', JSON.stringify({ id: cat.id, name: cat.name }));
  return cat;
}

function buildCSV({ categoryId }) {
  // Random unique slugs
  const slugA = `bulk-a-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 6)}`;
  const slugB = `bulk-b-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 6)}`;
  const slugBad = `bulk-bad-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 6)}`;

  const header = ['title', 'slug', 'price', 'manufacturer', 'description', 'mainImage', 'categoryId', 'inStock'].join(',');
  const row1 = ['Bulk Test Product A', slugA, 199, 'TestCo', 'Bulk A', 'productA.webp', categoryId, 1].join(',');
  const row2 = ['Bulk Test Product B', slugB, 299, 'TestCo', 'Bulk B', 'productB.webp', categoryId, 1].join(',');
  // Invalid row: missing categoryId and non-numeric price → should be captured as error
  const rowBad = ['Bulk Invalid Product', slugBad, 'abc', 'TestCo', 'Invalid item', 'bad.webp', '', 1].join(',');

  return { csv: [header, row1, row2, rowBad].join('\n'), slugs: { a: slugA, b: slugB, bad: slugBad } };
}

async function postBulkUpload(csvContent) {
  const form = new FormData();
  const blob = new Blob([csvContent], { type: 'text/csv' });
  form.append('file', blob, 'bulk.csv');

  const res = await fetch(`${API_BASE_URL}/api/bulk-upload`, { method: 'POST', body: form });
  const text = await res.text();
  let payload;
  try { payload = JSON.parse(text); } catch { payload = { raw: text }; }
  logStep('POST /api/bulk-upload', '201 Created with batch summary', `${res.status} ${JSON.stringify(payload).slice(0, 300)}...`);
...
async function postBulkUploadMissingFileShould400() {
  const res = await fetch(`${API_BASE_URL}/api/bulk-upload`, { method: 'POST' });
...
async function listBatches() {
  ...
  logStep('GET /api/bulk-upload', '200 OK with list', `${res.status} (count=${Array.isArray(data) ? data.length : 'n/a'})`);
...
async function putUpdateBatchItems(batchId, itemId, price, inStock) {
  ...
  const data = await res.json().catch(() => null);
  logStep(`PUT /api/bulk-upload/${batchId}`, '200 OK and item updated', `${res.status} ${JSON.stringify(body)}`);
  assert(res.status === 200, `Expected 200, got ${res.status}`);
  return data;
}

async function deleteBatch(batchId) {
  const res = await fetch(`${API_BASE_URL}/api/bulk-upload/${batchId}`, { method: 'DELETE' });
...
async function run() {
  ...
  logStep('Batch in DB', 'Exists with items and status set', JSON.stringify({
    id: dbBatch?.id,
  ...
  assert(['PENDING', 'COMPLETED', 'PARTIAL', 'FAILED'].includes(dbBatch.status), 'Invalid batch status');
  // With 1 invalid row we expect PARTIAL or at least errorCount > 0
  assert(dbBatch.status === 'PARTIAL' || (dbBatch.errorCount ?? 0) > 0, 'Expected partial success/errors');
  ...
  logStep('Product references', 'All productId exist', `checked ${createdItems.length}`);

  // 2) GET list
  section('2) List batches');
  const list = await listBatches();
  assert(list.some(b => b.id === batchId), 'Created batch not listed');
  ...
  const updatedProduct = await prisma.product.findUnique({ where: { id: updatable.productId } });
  logStep('DB product updated', 'price=1234, inStock=0', JSON.stringify({ price: updatedProduct?.price, inStock: updatedProduct?.inStock }));
  assert(updatedProduct && updatedProduct.price === newPrice, 'Product price not updated');
  ...
  const productIds = (await prisma.bulk_upload_item.findMany({ where: { batchId }, select: { productId: true } }))
    .map(x => x.productId)
    .filter(Boolean);
  ...
  const batchGone = await prisma.bulk_upload_batch.findUnique({ where: { id: batchId } });
  assert(!batchGone, 'Batch still exists after deletion');
  const itemsGone = await prisma.bulk_upload_item.findMany({ where: { batchId } });
  ...
  logStep('Product cleanup', 'Products removed', `checked ${productIds.length}`);
...
run()
  .catch(async (err) => {
    console.error('\n❌ Critical failure:', err && err.message ? err.message : err);
